...
app.add_middleware(
    ...
    allow_origins=[
        "*"
    ],  # Permet à Next.js frontend d'appeler l'API en local ou en production
...
class EstimationRequest(BaseModel):
    surface: float = Field(
        ..., description="Surface habitable en m2", ge=10.0, le=300.0
    )
    ...
    annee_visite: int = Field(
        2025, description="Annee de reference pour l'estimation", ge=2020, le=2030
    )
    ...
    def validate_code_postal(cls, v):
        ...
        if not (
            v_clean.startswith("75")
            or v_clean.startswith("77")
            or v_clean.startswith("78")
            or v_clean.startswith("91")
            or v_clean.startswith("92")
            or v_clean.startswith("93")
            or v_clean.startswith("94")
            or v_clean.startswith("95")
        ):
            raise ValueError(
                "Le code postal doit appartenir a la region Ile-de-France (75, 77, 78, 91-95)."
            )
        return v_clean
...
def read_root():
    ...
    cache_status = (
        "Charge avec succes" if LOOKUP_POSTAUX is not None else "Non disponible"
    )
...
def estimate_property(req: EstimationRequest):
    logger.info(
        f"Estimation demandee : {req.type_bien}, {req.surface}m2, {req.pieces}p a {req.code_postal}"
    )
    ...
    if "erreur" in res:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=res["erreur"]
        )
...
def agent_chat(req: ChatRequest):
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"L'agent ReAct LangChain a rencontre une erreur ou le serveur LLM est hors-ligne : {e}"
            )
            logger.info(
                "Basculement sur le moteur d'analyse de secours FallbackAgent..."
            )
