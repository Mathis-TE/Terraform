...
try:
    ...
    if lookup_path.exists():
        ...
        print(
            f"[SUCCESS] Table de correspondance geographique chargee : {lookup_path.name}"
        )
    else:
...
def outil_estimation_ml(
    surface: int,
    pieces: int,
    code_postal: str,
    type_bien: str,
    annee_visite: int = 2025,
    ...
    try:
        ...
        if (
            "departement" not in geo_info
            or pd.isna(geo_info["departement"])
            or not geo_info["departement"]
        ):
        ...
        if (
            type_bien.lower() == "maison"
            and "type_bien_Maison" in donnees_entree.columns
        ):
        ...
        return (
            f"Le modele statistique RandomForest estime la valeur de ce bien ({type_bien}, {surface}m2, "
