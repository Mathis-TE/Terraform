...
def run_training(sample_size=None):
    ...
    if not csv_path.exists():
        print(
            f"[ERROR] Impossible de trouver le dataset propre a l'emplacement : {csv_path}"
        )
        sys.exit(1)
    ...
    filtered_len = len(df)
    print(
        f"   -> {initial_len - filtered_len:,} lignes filtrees ({filtered_len / initial_len * 100:.1f}% conservees)."
    )
    ...
    if sample_size and sample_size < filtered_len:
        print(
            f"   [WARNING] Echantillonnage de {sample_size:,} lignes demande pour accelerer l'entrainement..."
        )
    ...
    print(
        f"   -> Entrainement complete en {duration:.1f} secondes ({duration/60:.1f} minutes)."
    )
