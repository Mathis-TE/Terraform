...
axes[0].hist(
    df["prix"] / 1e6, bins=60, color="#378ADD", edgecolor="white", linewidth=0.4
)
...
axes[0].axvline(
    ...
    linestyle="--",
    linewidth=1.5,
    label=f"Médiane : {median_prix:.2f}M€",
)
axes[0].legend()
...
axes[1].axvline(
    ...
    linestyle="--",
    linewidth=1.5,
    label=f"Médiane : {median_m2:,.0f} €/m²",
)
axes[1].legend()

plt.suptitle(
    "Île-de-France - Distribution des prix", fontsize=13, fontweight="bold", y=1.02
)
plt.tight_layout()
plt.savefig(
    Path(__file__).parent / "../docs/fig_distribution_prix.png",
    bbox_inches="tight",
    dpi=150,
)
plt.close()
...
axes[0].bar(
    prix_dept["departement"].astype(str),
    prix_dept["prix_m2"],
    color="#378ADD",
    edgecolor="white",
)
...
axes[1].bar(
    top_15["code_postal"].astype(str),
    top_15["prix_m2"],
    color="#D85A30",
    edgecolor="white",
)
axes[1].set_title(
    "Top 15 des codes postaux les plus chers d'Île-de-France",
    fontsize=12,
    fontweight="bold",
)
...
axes[2].bar(
    bottom_15["code_postal"].astype(str),
    bottom_15["prix_m2"],
    color="#1D9E75",
    edgecolor="white",
)
axes[2].set_title(
    "Top 15 des codes postaux les moins chers d'Île-de-France",
    fontsize=12,
    fontweight="bold",
)
...
plt.suptitle(
    "Analyse géographique des prix - Île-de-France",
    fontsize=14,
    fontweight="bold",
    y=1.01,
)
plt.tight_layout()
plt.savefig(
    Path(__file__).parent / "../docs/fig_prix_code_postal.png",
    bbox_inches="tight",
    dpi=150,
)
plt.close()
...
ax.set_title(
    "Corrélations entre les features (Complète et Entière)",
    fontsize=13,
    fontweight="bold",
)
plt.tight_layout()
plt.savefig(
    Path(__file__).parent / "../docs/fig_correlations.png", bbox_inches="tight", dpi=150
)
plt.close()
...
prix_temps = df.groupby(["annee", "type_bien"])["prix_m2"].median().reset_index()
fig_evol = px.line(
    prix_temps,
    x="annee",
    y="prix_m2",
...
del_agg = (
    df.groupby("departement_str")["score_delinquance"]
    .mean()
    .reset_index()
    .sort_values("score_delinquance", ascending=False)
)
...
axes[0].set_title(
    "Taux de Délinquance Moyen par Département (Normalisé sur 10)",
    fontsize=12,
    fontweight="bold",
    pad=15,
)
...
for p in axes[0].patches:
    ...
    axes[0].annotate(
        f"{height:.2f}",
        (p.get_x() + p.get_width() / 2.0, height + 0.1),
        ha="center",
        va="bottom",
        fontsize=10,
        fontweight="bold",
...
axes[1].set_title(
    "Distribution des scores Géorisques par Département",
    fontsize=12,
    fontweight="bold",
    pad=15,
)
...
plt.suptitle(
    "Analyse comparative de la Délinquance et des Risques environnementaux par Département en Île-de-France",
    fontsize=14,
    fontweight="bold",
    y=0.98,
)
...
# Sauvegarde
plt.savefig(
    Path(__file__).parent / "../docs/fig_risques_delinquance_departement.png",
    bbox_inches="tight",
    dpi=150,
)
plt.close()
